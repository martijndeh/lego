...
exports = module.exports = function (Lego) {
	return {
		...
		getDatabaseVersion: function getDatabaseVersion() {
			return Lego.sql(_templateObject).first().then(function (row) {
				if (row) {
					return row.version;
				} else {
					return 0;
		...
		createMigrationsTable: function createMigrationsTable() {
			return Lego.sql(_templateObject2).catch(function (error) {
				if (error.code == '42P06') {
		...
		migrate: function migrate(from, to) {
			...
			var _migrate = function _migrate(version, direction) {
				return Lego.transaction(function (lego) {
					...
					return Promise.resolve(migration[direction](lego, queue)).then(function () {
						...
						return lego.sql(_templateObject3, newVersion);
					});
				});
			};
