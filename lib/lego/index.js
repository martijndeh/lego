				...
				if (parameter instanceof Lego) {
					...
					for (j = 0, jl = parameter.length; j < jl; j++) {
						...
						if (isAppending) {
							...
							(_query2 = this.query).push.apply(_query2, _toConsumableArray(lego.query.slice(1, -1)).concat(['']));
						} else {
							var _query3;

							(_query3 = this.query).push.apply(_query3, [first + lego.query[0]].concat(_toConsumableArray(lego.query.slice(1, -1))));
						}
		...
		value: function _append(strings, parameters) {
			if (parameters.length) {
				...
				if (this.query.length) {
					var _query4, _parameters3;

					// TODO: Pop and add a new value instead?

					this._pasteString(strings[0]);
					(_query4 = this.query).push.apply(_query4, _toConsumableArray(strings.slice(1)));
