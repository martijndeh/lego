...
function createStateID(id, parents, row, columnNames) {
	if (row && columnNames) {
		return ['object'].concat(_toConsumableArray(parents), _toConsumableArray(columnNames.map(function (columnName) {
...
function parse(rows, definition) {
	...
	function parseColumn(object_, parents) {
		...
		Object.keys(object).forEach(function (propertyName) {
			...
			var isArrayOfStrings = Array.isArray(column) && column.every(function (column) {
			...
			if (isString || isArrayOfStrings) {
				var columnName = isArrayOfStrings ? column[0] : column;
				columns[columnName] = {
					name: propertyName,
			...
			if (column) {
				...
				if (column.name == 'id') {
					if (row[columnName] === null) {
						...
						var stateID = createStateID(row[columnName], column.parents, row, column.columnNames);
